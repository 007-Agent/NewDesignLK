...
export function Visit({ visit, patient, onRefresh, onGetMessage }: VisitProps) {
  ...
  const room = visit.room ? <div>(Кабинет: {visit.room})</div> : null;

  const doctorName = visit.doctorName ? <div>{visit.doctorName}</div> : null;
  ...
  return (
    <div className="visit-item">
      <div className="visit-item-header">
        ...
        {visit.active === 0 ? (
          <span className={`visit-status-badge green`}>Приём завершён</span>
        ) : (
          cancel
        )}
      </div>
